...
export default function Settings() {
  ...
  // employer (order + limit 1) to dodge legacy duplicates.
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .limit(1)
      .then(({ data }) => {
        const code = (data?.[0] as any)?.invite_code;
              ...
              {helperOpen && (
                <div className="px-5 pb-5 border-t border-black/5">
                  <div className="pt-4 space-y-4">
                    ...
                    {/* ── 6-digit invite code ──
                        The Helper enters this on /helper to bind. We render
                        each digit in its own pill so it's readable across the
                        room (worker showing their phone to employer, etc.).
                        Tap to copy. */}
                    {inviteCode && (
                      <div>
                    ...
                    <button
                      onClick={() => {
                        ...
                        const text = encodeURIComponent(
                          `Hi ${helperName}! 我用爱吃Aieats管理家里的菜单，你可以直接在上面查看今天的采购和备菜任务。${codeLine}\n\n${link}`
                        );
