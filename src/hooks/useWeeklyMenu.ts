  ...
  useEffect(() => {
    ...
    async function build() {
      ...
      try {
        ...
        // TICKET-094 — chat 偏好注入 prefScores 高权重 (chat 1.5× > swap 1.0×).
        // 老板拍板: chat + 使用数据是核心命中率来源. chip 答案直接调菜分数.
        // 不动 scoreForWeek 接口, 走 prefScores axis 4 学习曲线自动用上.
        try {
          const householdIdForChat = localStorage.getItem('nutri_household_id');
          const chatPrefs = await loadChatPreferences(supabase, userId, householdIdForChat);
          if (Object.keys(chatPrefs).length > 0) {
