...
Constant in `src/hooks/useWeeklyMenu.ts`. Currently **`v58`** (bumped 2026-05-22 by TICKET-020: §A lunch/dinner 切 slots[].primary 投影 (Home + WeeklyMenu) + §B weekStats edge fn 真接口接入 — deriveBadges 💪 channel nutrient match + bool fallback; v57 by TICKET-019: jsonb unwrap + Home fruit/breakfast slots[]; v55 by TICKET-018: 5-channel slots[].candidates[] + tagBadges[]; previous bumps v51→v52→v54 on TICKET-016/017; Smell 4 fix 2026-05-19 retired the two localStorage sentinels in favor of DB columns `user_weekly_menus.algo_version` + `cache_key`).
